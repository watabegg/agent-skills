...
この Skill は k16shikano 氏の Gist「日本語技術文書の文章規範」（https://gist.github.com/k16shikano/fd287c3133457c4fd8f5601d34aa817d）を元にしたものです。本 repo では Codex Skill として使うための frontmatter と、個人運用上の調整を加えています。
...
- 通常の質問回答、設計、実装方針、コードレビュー、調査結果、作業報告では、意味のまとまりごとに段落を作る。一文ごとの改行はしない。
- 技術書の章、草稿、記事、解説文を本文として執筆・推敲するとき、または差分で文単位の検討をしやすくしたいときだけ、一文ごとに改行してよい。
- 段落の区切りは空行で示す。ただし、短い回答では空行を増やしすぎず、近い内容は同じ段落にまとめる。
