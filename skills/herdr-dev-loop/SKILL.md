...
6. Confirm `herdr`, `git`, and the configured subordinate agent CLIs are available. Codex is the default fallback provider; Claude is optional unless selected for a role. `$codex-impl` remains an optional Worker compatibility protocol. Fresh 0.5.3 defaults set ordinary `reviewer.protocol`, `review.pre_final_protocol`, and `review.manual_final_protocol` to `$codex-review-multi-v2` and use the canonical six-lane Reviewer topology. `--review-protocol native` changes only ordinary review. To select the supported native pre-final path, set `pre_final_protocol = "native"` separately in `[defaults.review]` or a matching scope. Manual-final has no native override: `manual_final_protocol` accepts only `codex-review-multi-v2`. Any execution that selects the external protocol requires the pinned `externally-planned-v1` companion capability, and the shipped `release_ready=false` record blocks publication until an immutable companion distribution is supplied.
...
Mutating `hloop` commands enforce their own preflight. `pump`, `tick`, `worker start`, `worker harvest`, `worker seal`, `merge`, `validate`, `triage`, `gap start`, `gap harvest`, `reviewer start`, and `reviewer harvest` check the relevant Herdr environment, current branch, required commands, and non-loop dirty files before changing state.
...
When sending additional instructions to a running TUI, use `hloop worker message <task-id> --file <prompt.md>`, `hloop gap message <gap-id> --file <prompt.md>`, `hloop reviewer message <review-id> --file <prompt.md>`, or `hloop advisor message <advice-id> --participant-id P1 --file <prompt.md>`. Do not send prompts directly with `herdr pane run`; use direct pane delivery only while debugging the helper itself. The helper blocks common mistakes: shell panes, pending trust prompts, and busy agent sessions. For Codex it sends the structured start marker, body, and end marker in order, sends the body exactly once, exposes the staged tail, and sends at most two delayed Enter attempts. Claude and legacy marker-less messages use the single-send path.

If a message command exits nonzero, do not rerun the same message command and do not resend the body. `undelivered` proves that no body was typed and is the only state eligible for `hloop message drain`, but drain resends only to roles that are still running. For a terminal role, use the repository-pinned `message resolve ... --status superseded` command shown by `hloop message list`; never replay the body. A drain attempt that becomes `unknown` exits nonzero and must not be retried. `unknown` means the body may already be typed: rediscover it after a lost diagnostic or Manager restart with `hloop message list --status unknown`. Use the listed `hloop message submit <agent-id> <message-id>` only when the durable record authorizes Enter-only recovery. Otherwise inspect the pinned pane and explicitly record the observed `acknowledged`, `applied --result ...`, or `superseded` outcome with `hloop message resolve`; leave it `unknown` if the outcome cannot be established. Malformed message records, pending markers, and pending containers remain blocking and inspection-only. Any `undelivered`, `unknown`, or malformed Manager-message delivery blocks `finish` until it is safely drained, resolved, or superseded.
...
## 0.5.3 Planning, Candidate, And Review Epoch Gates

New loops use `state_format_version: 3` with `schema_revision: 3`. New task and result artifacts use `contract_schema_revision: 3`; migrated revision-2 artifacts remain historical evidence and require the explicit legacy acceptance path before merge. Review policy defaults to batch cadence, six externally planned Reviewer lanes, four Gap lanes, `max_fix_rounds: 2`, `max_patch_review_rounds_per_task: 2`, `audit_agent_budget: 12`, and complete-zero final certification. Migration preserves the stored cadence and finish behavior of legacy loops.
...
Before Worker dispatch, create and validate the Repository Impact Map, Task Risk Graph, coverage ledger, and Plan Gap artifact with `hloop planning check`. Planning identity binds the current plan, requirements, release scope, and source digests; stale or incomplete planning evidence stops dispatch. Worker implementation is a nonterminal candidate: `hloop worker submit` records revision-3 invariant, regression, self-review, residual-risk, and unrun-check evidence, then commit mode uses `worker candidate-seal` and handoff mode uses Manager sealing. Required Patch Review must target that exact candidate SHA, attempt, task contract, and artifact digest before finalization.

Reviewer and Gap work is collected in an immutable review epoch. Capacity reservation counts Coordinator, lane, verifier, and Patch Reviewer processes against one epoch-wide budget; expired work remains quarantined until process exit or forced abort is confirmed. `hloop triage epoch` records the complete collection first, then permits one explicit approval transition and deterministic write-ahead task materialization. Classification conflicts, incomplete required executions, stale successor identity, exhausted round budgets, and ambiguous crash recovery stop instead of guessing.

`codex-review-multi-v2` is an external companion and is the fresh 0.5.3 default for ordinary review, pre-final, and manual-final. The canonical Reviewer topology has six lanes. `--review-protocol native` applies only to ordinary review and does not change either final-review key. The supported native pre-final path requires a separate `pre_final_protocol = "native"` setting. Manual-final has no native override and accepts only `codex-review-multi-v2`. Any execution that selects the external protocol must provide a pinned capability record for `externally-planned-v1`; absence, version drift, digest drift, or a missing capability fails closed. The shipped `release_ready=false` record blocks publication until an immutable distribution is supplied; the release dependency and Codex/Claude install handshake are defined in `release-dependencies.json` and `references/migration-install.md`.
...
The initial semantic ACK performs a real Git metadata probe and binds `commit` or `handoff` completion mode to the attempt. Revision-3 Workers submit the exact candidate before finalization. In commit mode the Worker commits product changes, runs `worker submit --completion-mode commit`, passes required Patch Review/full-suite gates, and finalizes without `--handoff`. In handoff mode the Worker does not stage or commit, submits with `--completion-mode handoff`, and finalizes with `--handoff`; Manager sealing performs the Git transaction. Validation results use the explicit `passed`, `failed`, or `blocked` enum, and every required result must be `passed` for status `done`.

A Codex `workspace-write` Worker that cannot write Git metadata uses the approved handoff mode, leaving in-scope product changes and `result.md` uncommitted. Manager runs `hloop worker seal <task-id> [--attempt-id <id>] [--validation-command ...]` to close or confirm the quiesced pane, validate the active attempt, semantic ACK, candidate identity, branch/worktree identity, full write scope, Patch Review, and validation evidence, then commits the handoff once at Worker HEAD. See `references/worker-contract.md` and `references/artifact-contract.md`.
...
For a loop created by an older runtime, run `hloop migrate --dry-run` and then `hloop migrate --apply` before continuing. Schema 3.3 migration is a prepared transaction with per-artifact digests and atomic replacement; interrupted work resumes with `--resume`. `--rollback` is allowed only before the first recorded 0.5.3 mutation and only when archive and marker identity remain canonical. `pump` may reach `ready_to_finish`, but it never marks the loop done; run `hloop finish` to recheck planning, candidate, validation, review epoch, remediation, Manager QA, cleanup, merge, agent, and checkout gates against the current integration SHA.
...
For new 0.5.3 loops, dispatch up to three Workers in non-overlapping scopes only after current planning evidence passes. Review at batch boundaries after the batch closes, collect required Reviewer and Gap executions for the same epoch before remediation, and keep Patch Review task-local. The explicit convergence and manual-final commands own fixed-target final review; the scheduler must not silently replace them with an ordinary Reviewer. A legacy or explicitly configured `merge-count` policy may still open review/gap gates from validated merge counters. Manager must not mutate the inspected integration head while an audit epoch is collecting evidence.

Worker and review execution protocols have distinct defaults:
...
- Fresh 0.5.3 ordinary review defaults to `$codex-review-multi-v2` with the canonical six-lane Reviewer topology. `--review-protocol native` selects the HLoop Native Review Protocol only for ordinary review.
- Pre-final also defaults to `$codex-review-multi-v2`; select its supported native path separately with `pre_final_protocol = "native"` in `[defaults.review]` or a matching scope.
- Manual-final defaults to and only supports `$codex-review-multi-v2`; it has no native override.
