...
class CanonicalizePathTests(unittest.TestCase):
    def test_expands_home(self):
        result = config.canonicalize_path("~/workspace", base="/tmp")
...
class ValidateConfigTests(unittest.TestCase):
    ...
    def test_valid_full_example_from_spec(self):
        data = {
            ...
            "scope": [
                {"path": "~/workspace", "worker": {"provider": "claude", "model": "sonnet"}},
                {
                    "path": "~/workspace/vdr",
                    "reviewer": {
                        "mode": "dual-swarm",
...
class MatchScopesTests(unittest.TestCase):
    def test_repo_scope_matches_ancestor_of_repo_root(self):
        scopes = [{"path": "/home/user/workspace"}]
        matched = config.match_scopes(
            scopes, repo_root=Path("/home/user/workspace/vdr"), cwd=Path("/home/user/workspace/vdr/sub")
        )
        self.assertEqual(len(matched), 1)
        self.assertEqual(matched[0].match_kind, "repo")

    def test_cwd_scope_requires_explicit_match(self):
        scopes = [{"path": "/home/user/workspace/vdr/sub", "match": "cwd"}]
        matched = config.match_scopes(
            scopes, repo_root=Path("/home/user/workspace"), cwd=Path("/home/user/workspace/vdr/sub")
        )
        self.assertEqual(len(matched), 1)
    ...
    def test_non_ancestor_scope_does_not_match(self):
        ...
        matched = config.match_scopes(
            scopes, repo_root=Path("/home/user/workspace"), cwd=Path("/home/user/workspace")
        )
        self.assertEqual(matched, [])

    def test_no_repo_root_skips_repo_scopes(self):
        scopes = [{"path": "/home/user/workspace"}]
        matched = config.match_scopes(scopes, repo_root=None, cwd=Path("/home/user/workspace"))
        self.assertEqual(matched, [])

    def test_sorted_shallow_to_deep(self):
        scopes = [
            {"path": "/home/user/workspace/vdr"},
            {"path": "/home/user/workspace"},
        ]
        matched = config.match_scopes(
            scopes, repo_root=Path("/home/user/workspace/vdr"), cwd=Path("/home/user/workspace/vdr")
        )
        self.assertEqual([m.canonical_path for m in matched], [Path("/home/user/workspace"), Path("/home/user/workspace/vdr")])
