...
For a herdr-dev-loop release, unit tests and selftest are not substitutes for the scenario runners. Run `tests/run_synthetic_e2e.py --expected-integration-sha "$(git rev-parse HEAD)" --json` from a clean checkout and preserve its structured scenario results outside the skill source subtree. The runner re-executes release scenarios from a private detached worktree of that SHA. The child may attest only `execution_verified`; final `checkout_identity.verified=true` requires the source-checkout parent to remove the private worktree and set `parent_cleanup_attested=true` with `snapshot_cleanup.status=passed`. Run `tests/run_provider_e2e.py --provider <codex|claude> --json` only with an authenticated disposable session; the provider runner performs the live read-only probe by default. When credentials or a session are unavailable, use the runner's explicit `--allow-skip --skip-reason <reason>` path; a skipped result proves non-execution and does not count as a live provider pass.
