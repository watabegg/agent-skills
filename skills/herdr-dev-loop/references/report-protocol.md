...
`hloop agent report` accepts the report body as a schema-validated JSON object via `--file <path>` or `--stdin`, or as the individual `--type`/`--stage`/`--summary`/`...` flags kept as compatibility input; the two input styles are mutually exclusive per call. Before ever reaching the broker or the fallback spool, the client atomically persists the complete normalized client event in a `0600` role-local outbox keyed by run/role/attempt and by the semantic report fields excluding the generated timestamp. An implicit invocation without `--event-id` reuses only an unconfirmed pending delivery, closing the broker-commit/client-interruption window without duplicating an event, inbox projection, or wake. Broker acceptance or successful fallback spooling marks that exact envelope confirmed; a later implicit invocation with identical semantic fields therefore receives a distinct event ID. An explicit `--event-id` retry instead reuses the exact retained envelope, including after confirmation, when its semantic digest matches; conflicting semantic content is rejected locally before broker access.
