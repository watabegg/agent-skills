...
After collection, the Manager records all normalized candidates before approving one deterministic remediation batch. Classification conflicts stop approval. A convergence manifest that is incomplete, stale, or still contains verified actionable findings cannot pass. Normal remediation and task-local Patch Review each retain a configured default maximum of two rounds.

One accepted structured user decision may grant exactly one task-local third Patch Review round through `hloop patch-review authorize-extra-round TASK --decision-id D... --authorization-input-id U...`. The command requires two retained counted reviews at the configured base limit, the last review's exact blocking candidate SHA and semantic finding fingerprints, an accepted decision whose user response and resolution select the same option, the exact affected task, and exact `source_findings`, plus a local `source=user` input artifact whose prompt digest matches `inputs_index`. The canonical authorization binds the current task contract and becomes consumed only by the matching round-3 record. Missing, stale, mismatched, previously authorized, or reused evidence fails closed; setting the configured maximum to three is not an alternative. History is preserved, no task is created automatically, and a fourth round is impossible even when round 3 still has findings.
