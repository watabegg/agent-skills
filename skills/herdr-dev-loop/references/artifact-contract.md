...
If a Manager `--validation-command` fails, seal rolls the worktree back to its pre-validation dirty state byte for byte before raising. Every path dirty before validation ran (including tracked files already dirty from the Worker's own edits) is restored from a snapshot taken just before the first command ran -- exact content, executable mode, symlink target, and deletion state. Seal also records every real pre-validation directory because Git porcelain does not report directories or clean tracked siblings. Rollback removes newly reported leaf files individually, restores clean tracked files from `HEAD`, and prunes only directories proven absent from that directory snapshot, deepest-first with `rmdir`; it never recursively deletes an ancestor. This removes whole new output trees and even new empty directories while preserving existing tracked/pre-validation directories, clean tracked siblings, and non-empty concurrent content. `shutil.rmtree` remains limited to restoring a snapshotted file/symlink path that validation replaced with a directory, where that directory is itself proven to have replaced non-directory content. A failing validation command therefore never leaves the worktree unable to match the handoff's declared `changed_files`, and a retried `hloop worker seal <task-id>` is not stuck on a permanent "stale handoff artifact" error.
...
If content is written or re-staged at the same path after the tree checkpoint, neither normal seal nor crash resume can put it into the commit because `commit-tree` reads the recorded tree object, not the mutable index. After the ref CAS succeeds, seal reconciles the index with a mixed reset to the new `HEAD`; it never uses a hard reset, so concurrent worktree content survives as an unstaged change. If the Manager process dies after the commit id is persisted or after the ref CAS wins, the next `hloop worker seal <task-id>` verifies the recorded commit's tree, single parent, and message, completes or recognizes the same CAS, and finishes index reconciliation and state cleanup idempotently. Before the CAS, crash resume still requires the staged path set and recomputed `git write-tree` to match the durable transaction exactly. Foreign staged changes, a stale attempt, a tree-identity mismatch before commit-object creation, a transaction without the tree checkpoint, an unexpected commit object, or concurrent branch movement all fail closed; tampered content is never committed.
