...
Manager validation commands for `status: done` never run in the Worker's own worktree. Seal first builds a disposable, git-aware copy of it (`build_validation_snapshot`): every path is reproduced byte for byte -- tracked, untracked, and ignored files, executable bits, deletions, and empty directories -- except the worktree's own `.git`, which is replaced with a real one obtained by a local, no-hardlink `git clone --no-checkout` of the worktree's current `HEAD` (with an explicit `git read-tree HEAD` guaranteeing the index matches it), so ordinary `git status`/`git diff` behave identically inside the copy. Symlinks get individual treatment: one whose target resolves (after fully following any symlink chain, not just a lexical `..` collapse) inside the worktree is either copied unchanged (a relative target, since the reproduced layout keeps it resolving correctly) or rewritten to the snapshot's equivalent absolute path (an absolute target, so following it can never land back in the original worktree); one whose target resolves outside the worktree cannot be safely isolated at all, so building the snapshot fails closed before any command runs. Every `--validation-command` then runs against this snapshot, in order, stopping at the first failure exactly as before -- but whatever it mutates, formats, or generates stays inside the snapshot, which is discarded (success or failure) once the command sequence finishes. The Worker's worktree is therefore never touched by validation: there is nothing to roll back on failure, and a passing command's side effects can never leak into the sealed commit -- seal always lands exactly the tree recorded before validation ran. A failing validation command can never leave the worktree unable to match the handoff's declared `changed_files`, so a retried `hloop worker seal <task-id>` is never stuck on a permanent "stale handoff artifact" error.

Only after every check passes does seal stage the dirty paths and create one commit, `ai-loop(<task-id>): seal worker handoff (<attempt-id>)`, in the Worker's own worktree. The namespaced result artifact is always force-staged (`git add -f`) so it lands in the commit even in a repository that gitignores `.ai`; every other dirty path keeps normal `git add -A` semantics (respecting `.gitignore`, honoring deletions). The result is indistinguishable from a Worker that committed its own result: `hloop worker harvest` and `hloop merge` require no changes to consume it.

Before staging, seal records the exact attempt, result path, staged paths, commit message, attached Worker branch ref, and expected parent commit it is about to use in Worker state. After staging succeeds and is re-verified against the scanned scope, seal computes `git write-tree` over the staged index and saves that hash into the same transaction record as a second durable checkpoint. The index is not the commit source after this point: seal creates the commit object with `git commit-tree <recorded-tree> -p <expected-parent>`, persists that commit id, and advances `refs/heads/<worker-branch>` with `git update-ref <ref> <new-commit> <expected-parent>`. The old-value argument is an atomic compare-and-swap; concurrent ref movement fails closed instead of silently changing the parent.

If content is written or re-staged at the same path after the tree checkpoint, neither normal seal nor crash resume can put it into the commit because `commit-tree` reads the recorded tree object, not the mutable index. After the ref CAS succeeds, seal reconciles the index to the landed commit with `git read-tree <commit>` -- an index-only operation -- never a mixed (or hard) reset, which would also set the current branch ref to `<commit>` as a side effect. That side effect is harmless when nothing else has moved the ref, but if another process advances it during this reconciliation window, a mixed reset would silently move it back and discard that concurrent commit's ref position; `read-tree` cannot do that because it never touches HEAD or the branch ref. Seal instead verifies the branch ref still equals the recorded commit immediately before and after calling `read-tree`; if it does not (a concurrent advance won the race), seal fails closed without touching the ref and without clearing the seal transaction, so both the competing commit and the recorded transaction survive for inspection. If the Manager process dies after the commit id is persisted or after the ref CAS wins, the next `hloop worker seal <task-id>` verifies the recorded commit's tree, single parent, and message, completes or recognizes the same CAS, and finishes this same ref-safe index reconciliation and state cleanup idempotently -- subject to the identical concurrent-advance check. Before the CAS, crash resume still requires the staged path set and recomputed `git write-tree` to match the durable transaction exactly. Foreign staged changes, a stale attempt, a tree-identity mismatch before commit-object creation, a transaction without the tree checkpoint, an unexpected commit object, or concurrent branch movement (before the CAS or during index reconciliation, on a fresh seal or a crash resume alike) all fail closed; tampered content is never committed.
