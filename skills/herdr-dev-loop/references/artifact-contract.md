...
If a Manager `--validation-command` fails, seal rolls the worktree back to its pre-validation dirty state byte for byte before raising: every path dirty before validation ran (including tracked files already dirty from the Worker's own edits) is restored from a snapshot taken just before the first command ran -- exact content, executable mode, symlink target, and deletion state, using `shutil.rmtree` first if validation replaced a file/symlink with a directory -- and any path newly introduced by validation (including a whole new untracked directory) is removed outright. A validation command that mutates the tree on failure therefore never leaves the worktree unable to match the handoff's declared `changed_files`, so a retried `hloop worker seal <task-id>` is not stuck on a permanent "stale handoff artifact" error.
...
Before staging, seal records the exact attempt, result path, staged paths, and commit message it is about to produce in Worker state. After staging succeeds and is re-verified against the scanned scope, seal computes `git write-tree` over the now-staged index and saves that hash into the same transaction record as a second, later durable checkpoint. If the Manager process dies after this checkpoint lands but before `git commit`, the next `hloop worker seal <task-id>` recognizes the staged index, recomputes `git write-tree`, and only completes the same commit directly if that hash matches exactly -- not just that the staged path names match. This detects same-path tampering or re-staging between the crash and the resume, not only a changed path set. A staged index that does not exactly match a recorded transaction this way (foreign staged changes, a stale transaction for a different attempt, a tree-identity mismatch, or a crash before the tree-identity checkpoint itself was durably recorded) still fails closed with the "already has staged changes" error above -- in every such case nothing is committed, so a corrected seal (including Manager validation) must run again from scratch.
