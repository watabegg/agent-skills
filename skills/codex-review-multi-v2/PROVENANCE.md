...
- bind attestation to a start-of-review Git and worktree fingerprint, including branch-mode dirty bytes plus untracked file kinds and canonical Git executable modes, include those bytes in the declared review scope, force Git to reveal ignored submodule state, and reject dirty submodules whose inner bytes are not pinned by the superproject;
